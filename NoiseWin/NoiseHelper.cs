...
namespace NoiseWin
{
    internal static class NoiseHelper
    {
        ...
        /// <summary>
        /// Получение нормативного уровня шума
        /// </summary>
        /// <param name="tableType">Тип таблицы (2.1 или 2.3)</param>
        /// <param name="level">Уровень</param>
        /// <param name="roomType">Тип помещения или территории</param>
        /// <returns>Нормативный уровень шума</returns>
        ...
        /// <summary>
        /// Поправки к нормативному уровню шума
        /// </summary>
        /// <param name="addNoise">Дополнительные характеристики помещения или территории</param>
        /// <param name="tableType">Тип таблицы (2.1 или 2.3)</param>
        /// <returns></returns>
        ...
        /// Формула 2.3
        /// </summary>
        /// <param name="map">Карта-схема, необходима для поска связанных перегородок</param>
        /// <param name="mapElement">Элемент карты</param>
        /// <param name="level">Уровень</param>
        /// <returns>Значение звукового давления в расчетной точке</returns>
        public static double GetPowerInPoint(Map map, NoiseMapElement mapElement, int level)
        {
            ...
            // базовое значение + изолирующая способность перегордок + защитный экран
        ...
        /// <returns>Требуемое снижение уровня шума</returns>
        ...
        /// <summary>
        /// Максимальная разница из п. 2.5.4
        /// </summary>
        public const int MaxDifference = 8;

        /// <summary>
        /// Формирование расчетной таблицы
        /// </summary>
        ...
        public static string Calc(Map map)
        {
            ...
            // добавляем заголовк и шапкутаблицы
            ...
            var number = 1;
            // расчитываем уровни звуквого давления для всех источников шума и добавляем их в таблицу
            ...
            // поправки к нормативным уровням шума
            var deltas =
            ...
            // добавляем в таблицу нормативные уровни шума
            sb.AppendFormat(
                ...
                normalNoiseLevels[7]);
            number++;
            // добавляем в таблицу поправки уровня шума
            sb.AppendFormat(
                ...
                deltas[7]);
            number++;
            // добавляем в таблицу допустимые уровни шума
            sb.AppendFormat(
            ...
            number++;
            // расчитываем количество источников шума которое удовлетворяют условию из п. 2.5.4
            var countHasDiff =
                powerInAllPoints.Count(p => p.Where((p1, i) => allowableNoiseLevels[i] - p1 <= MaxDifference).Any());

            // рассчитываем отклонения от допустимого уровня шума
            ...
            // добавляем отклонения от допустимого уровня шума в таблицу
        ...
        /// <summary>
        /// Формирование таблицы "Источники шума"
        /// </summary>
        /// <param name="mapElements">Список источников шума</param>
        /// <returns></returns>
